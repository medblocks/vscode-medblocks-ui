...
export async function activate(context: vscode.ExtensionContext) {
  ...
  vscode.commands.registerCommand(
    ...
    async (node: TemplateItem) => {
      console.debug(node.tree.inContext);
      vscode.env.clipboard.writeText(node.tree.inContext ? node.copy() : "");
      if (node.type === "templateItem") {
        vscode.window.showInformationMessage(`Copied ${node.displayLabel}.`);
      } else {
        vscode.window.showInformationMessage(
          `Copied snippet ${node.displayLabel}.`
        );
      }
    }
  );
  ...
  //   "templateTree.copyContext",
  //   (node: TemplateItem) => {
  //     vscode.env.clipboard.writeText(node.treeItem.contextValue);
  //     console.debug();
  //     vscode.window.showInformationMessage(
  //       `Copied Context for ${node.displayLabel}.`
  //     );
  //   }
  // );
